...
function searchInit() {
    ...
    $.ajax({
        url: '/srh/public/coursesauditoria/table', 
        type: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = `/srh/public/coursesauditoria/edit/${object.id_auditoria}`;
...
function deleteCourse(id) {
    $.ajax({
        url: '/srh/public/coursesauditoria/delete/' + id,  // Verifica que esta ruta sea correcta
        type: 'DELETE',
        data: {
            _token: token  // Incluye el token CSRF
        },
        success: function(response) {
            window.location.href = '/srh/public/coursesauditoria/list';  // Redirigir a la lista de cursos
        },
